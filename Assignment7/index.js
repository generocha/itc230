...
app.get('/api/delete/:name', (req,res, next) => {
    Brewery.remove({"name":req.params.name }, (err, result) => {
        if (err) return next(err);
...
app.post('/api/add/', (req,res, next) => {
    ...
    if (!req.body._id) { // insert new document
        ...
        Brewery.updateOne({name:req.body.name, address:req.body.address, city:req.body.city, state: req.body.state, zip: req.body.zip, founded: req.body.founded}, {upsert: true }, (err, result) => {
            if (err) return next(err);
            if (result.upserted) {
